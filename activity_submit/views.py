...
def index(request):
    ...
    if not SystemControl.objects.all()[0].can_submit:
        ...
        return render(request, 'time_out_submit.html', context=context)
    # 正常流程
    if request.method == 'POST':
        ...
        if SystemControl.objects.all()[0].team_size == 1:
            return render(request, 'single_index.html', context=context)
        else:
            return render(request, 'index.html', context=context)


# 报名表
def submit_form(request, teammate):
    # 判断是否可以报名
    # 如果不可以，引导此时间段不可以报名提示页面
    if not SystemControl.objects.all()[0].can_submit:
        # 打包QQ群信息
    ...
    if request.method == 'POST':
        ...
        if len(Student.objects.filter(stu_id=stu_id)) > 0:
            ...
            # 引导前端页面
            return render(request, 'error_multiple_submit.html', context=context)
